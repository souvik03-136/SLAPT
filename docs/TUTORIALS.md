...
The green play button in the top bar. You should hear a basic four-on-the-floor pattern.
...
`kick pattern [1, 2.75, 3]` places kicks at beat 1, beat 2.75 (the "and" of beat 2), and beat 3. This is a classic lo-fi syncopation. The Timeline panel at the bottom will show you exactly where each kick lands.
...
`bass walking the roots` means the bass plays the root note of each chord. `follow chord progression` locks it to whatever progression is declared in the chords block. The bass will always be one octave below the chord roots.
...
`vinyl crackle at 20% volume` — the percentage controls how loud the crackle layer is relative to the mix. Try values between 10–30%.

---

## Tutorial 7 — Global Modifiers (3 minutes)

Modifiers are the fastest way to shift the entire vibe of a track. Add them at the end of your file, after all blocks.
...
This is the full "Midnight Study Session" pattern. Each section instructs the engine how to build up and break down the layers.
...
Warnings do not block playback. Errors do. The status dot in the editor header shows green (ok), yellow (parsing), or red (error).
...
`random(0.7 to 0.9)` randomizes the snare velocity between 70% and 90% on every hit. The range is `0.0` (silent) to `1.0` (full velocity). For a more dynamic feel try `random(0.5 to 1.0)`. For a subtle variation try `random(0.8 to 0.95)`.
