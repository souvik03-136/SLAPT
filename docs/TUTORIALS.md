...
`@genre lofi-hiphop` sets the vibe and unlocks genre-aware defaults — SLAPT warns you if your tempo or notes feel off for the genre you've picked. Supported genres: `lofi-hiphop`, `boom-bap`, `house`, `techno`, `dnb`, `ambient`, `trap`.
...
**Your code is auto-saved.** Every keystroke writes to `localStorage`. If you refresh or close the tab and come back, your code is still there. No manual save. No lost work.
...
**Beat validation:** SLAPT checks every beat in your pattern array. If you write `kick pattern [1, 2.75, 5]`, you'll get a `BEAT_OUT_OF_RANGE` error on the `5` specifically — with the exact line, the context (`kick pattern`), and a suggestion for what to try instead. The same validation applies to `snare on` and `hihat open on` beats.
...
**Beat validation works here too.** If your time signature is 3/4 (see Tutorial 13), `hihat open on 4` will trigger a `BEAT_OUT_OF_RANGE` error — beat 4 doesn't exist in 3/4.
...
`progression Am7 -> Fmaj7 -> Dm7 -> E7` — four chords, one per bar, looping. `->` is two characters you can type on any keyboard.
...
`vinyl crackle at 20% volume` — two synthesized layers: a quiet brown noise surface hum and random pop bursts that fire at irregular intervals, simulating an actual record. Try 10–30%; above 30% it becomes very noticeable.

`rain sounds softly in background` — three brown/pink noise layers filtered to different frequency bands (low rumble, mid splash, high sparkle), blending into a rain-in-a-room texture. It should sit well under the mix and add a sense of space.
...
The error panel shows: `Beat 5 doesn't exist in 4/4 time` with context `"kick pattern"` so you know exactly which line caused it, plus suggestions for what to try instead.
...
Beat validation range changes automatically. In 3/4, beats 1–3 are valid — beat 4 triggers `BEAT_OUT_OF_RANGE`. In 5/4, beats 1–5 are valid. The timeline shows the correct number of beat markers. MIDI export embeds the correct time signature meta event.
...
Drag the `.mid` file into Ableton, Logic, FL Studio, GarageBand, or any other DAW. Drums land on channel 10 (the General MIDI standard), so most drum kits will auto-map correctly. Chords and bass will need an instrument assigned.
...
**Copy button:** The icon in the top-right corner of the editor copies your entire code to the clipboard in one click. The icon briefly shows a checkmark to confirm. Use it to paste your code into a text file, share it, or back it up anywhere.

**Auto-save:** Every keystroke saves your code to the browser's `localStorage` under the key `slapt_code_v1`. Closing the tab, refreshing, or restarting Docker — your code comes back automatically.
...
Writing no `snare` line means no snare plays. Writing no `hihat` line means no hihat plays. SLAPT only plays what you explicitly write.
